...
- 試合終了（FT）を取得したら追跡をやめ、表示も即座に「試合終了」へ切り替わる
- 同期が追いつくまでは、ライブのスコアで結果を表示する

得点者は試合中、GitHub Actions 側が公式の試合結果ページを1分おきに確認し、
増えたら `data/fixtures.json` を更新する（`node scripts/sync.mjs --watch`）。
